...
from tokenDEF import Token, str_attr_token, SEP

TEST_LEXICAL_ERRORS = True

def verify_seps(seps):
    for c in seps:
        assert c in SEP, ("irrelevant test:" + repr(seps) + " should contain only spaces and not " + repr(c))


def verify_without_seps(lexeme):
     for c in lexeme:
        assert not (c in SEP), ("irrelevant test:" + repr(lexeme) + " should not contain spaces" + repr(c))
